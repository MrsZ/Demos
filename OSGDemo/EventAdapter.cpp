...
EventAdapter::EventAdapter(QWidget* parent,const char* name,
	                           const QGLWidget* shareWidget,
							   Qt::WindowFlags flag):QGLWidget(parent,shareWidget,flag)
{
...
void EventAdapter::keyPressEvent(QKeyEvent* event)
{
	...
	m_pGraphicsWindow->getEventQueue()->keyPress((osgGA::GUIEventAdapter::KeySymbol)*(event->text().toLatin1().data()));
}

void EventAdapter::keyReleaseEvent(QKeyEvent* event)
{
	setKeyboardModifiers(event);
	m_pGraphicsWindow->getEventQueue()->keyRelease((osgGA::GUIEventAdapter::KeySymbol)*(event->text().toLatin1().data()));
}
...
void EventAdapter::wheelEvent(QWheelEvent* event)
{
	...
	if(event->delta() > 0)
	{
		m_pGraphicsWindow->getEventQueue()->mouseScroll(osgGA::GUIEventAdapter::SCROLL_UP);
	}
	else
	{
		m_pGraphicsWindow->getEventQueue()->mouseScroll(osgGA::GUIEventAdapter::SCROLL_DOWN);
	}
}
